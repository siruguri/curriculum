---
layout: page
title: "Programming In 100 Minutes"
sidebar: true
pdf: false
alias: [ /programming-in-100-minutes, /programming-101 ]
---

Programming is the job of telling a computer what to do. There are many functions a typical computer - like your laptop or your phone - performs, and you can learn to program many of these pretty easily. Programming consists of giving a computer a set of instructions in a "language" that has words and a grammar that is understood, or "interpreted," by it.

In this brief introduction we'll look at the key ideas you need to get started with knowing how to program.

0. Programming: A Brief History and Some Concepts
...
<div class="note">
<p>This tutorial is open source. If you notice errors, typos, or have questions/suggestions, please <a href="https://github.com/JumpstartLab/curriculum/blob/master/source/projects/programming-in-100-minutes.markdown">submit them to the project on Github</a>.</p>
</div>

## 0. Programming: A Brief History and Some Concepts

People have been programming machines for much longer than what we know today as computers have
existed. The history of computing devices that ran on sets of instructions is often traced back to
the work done by Blaise Pascal, a French mathematician, on <a
href="http://en.wikipedia.org/wiki/Pascal%27s_calculator">a mechanical device that added and
multiplied numbers</a>. It was a primitive calculator, and like most calculators, the instructions
were fed to it in the simple form of the numbers that had to be added or multiplied. Its only output
was the result of the calculation.

In the 19th century, mechanical looms became common based on the design of a weaving head designed
by the Frenchman <a href="http://en.wikipedia.org/wiki/Joseph_Marie_Jacquard">Joseph Marie Jacquard</a>. The heads were manipulated through cards that had holes punched in them which allowed
the threads to be moved to different heights, so that the loom could pick up the right thread for
each row in the weave. This made it possible to produce complex weave patterns with very few errors,
compared to having it done by hand.

Cards with holes punched in them ("punch cards") were common in the twentieth century as well, but
they were replaced by electrical and electronic devices as the primary means of conveying a set of
instructions to a computing device or computer. Instead of manipulating mechanical arrangements, and
<a href="http://www.columbia.edu/cu/computinghistory/census-tabulator.html">then electrical circuits</a> , by using the sequence of holes in the punch cards, new devices were built where the
inputs were electrical switches. 

The enormous advantage of such switches was that the sequence of inputs could be altered at will; a
punch card once constructed couldn't be changed in any way. The first such computers built during
the Second World War started what we now think of as the standard design of computers - inputs
coming from switches (today, "keyboards") leading to outputs on electrical systems (today,
"monitors.")

This very hurried history jumps through many aspects of how modern computers came to be built the
way they are, but that is mostly not relevant to learning how to program. This is largely because
through a great deal of research and experimentation, the computing industry has become very good at
a concept called "abstraction," whereby the complexities of the architecture of a computer have been
hidden and <em>abstracted</em> away by progressive layers of instructions that make it easier to
address the computer using a language that is a lot more like the languages humans use to communicate with each other (though we still have a long way to go!)

The simplest programming instructions can be those that turn individual electronic circuits inside
your computer on and off. A typical laptop packs millions of these circuits, however, so
manipulating them individually is out of the question. While it's not unheard-of for some master programmers to continue to work with languages that manipulate these circuits, even they will be writing these instructions to  manipulate the behavior of collections of thousands of circuits, commonly packaged as devices called <em>chips</em>, <em>microchips</em> or <em>integrated circuits (ICs)</em>, at one go.

However, most programmers who write apps for phones and the Web use languages that are many <em>levels of abstraction</em> higher. If you want to display a line of text in a browser, for example, you will type a "Web address" into the browser's address bar (which is one high-level instruction,) and then write a web page in another language called HTML, which is another high-level set of instructions. HTML lets you specify what fonts to use, whether to show them in boldface, etc. You didn't have to write instructions necessary to start the browser in the first place, to let it figure out where the web page you wrote is stored, to read that page from your hard drive, to draw the text onto the screen, and so on. The few instructions that you did write were translated appropriately by a series of "interpreters" to perform the task you intended - showing text inside a browser window.

In the rest of this guide, we'll walk you through some of the concepts behind how interpreters for most programming languages work. Most intepreters share some common concepts that are simply written in different way, much as how different human languages will refer to the same idea but using different words (the Spanish for "dog" is "perro," the French for "cat" is "le chat," and so on.)

## 1. Instructions & Interpreters

All programming languages have a few concepts in common that are similar to rules for languages in human speech, which are typically referred to as "natural languages." There is a fixed vocabulary that defines the languages, this vocabulary can be arranged in a specific way, called the <em>syntax</em> of the language   that approximates, even if very slightly, natural human speech.
